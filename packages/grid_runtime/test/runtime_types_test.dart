...
void main() {
  group('RuntimeConfig', () {
    test('defaults: long-lived, empty args/env, no startup hint', () {
    ...
    test('value equality (freezed) — the diff primitive', () {
      const a = RuntimeConfig(workDir: '/w', command: 'claude', args: ['-p']);
      const b = RuntimeConfig(workDir: '/w', command: 'claude', args: ['-p']);
      expect(a, equals(b));
      expect(a.copyWith(workDir: '/x'), isNot(equals(b)));
    });
  });

  group('RuntimeCapabilities', () {
    test('subprocess preset is honest: liveness+output, no attach/activity', () {
  ...
  group('RuntimeEvent (sealed union)', () {
    test('shared name getter demuxes every variant', () {
      final events = <RuntimeEvent>[
        const RuntimeEvent.sessionStarted(name: 's1', pid: 1, pgid: 2),
