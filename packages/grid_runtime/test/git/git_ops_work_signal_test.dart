...
Future<GateOutcome> probe(
  ...
  _CannedGit(porcelain),
).hasUncommittedWork('/w', excluding: excluding);

void main() {
  group('ADR-0006 D3 Gate 1 is UNCHANGED with no exclusion (reap)', () {
    test(
      'grid residue with NO exclusion is still WORK (a reap must refuse)',
      () async {
    ...
    test('a failed probe is probeError, exclusion or not', () async {
      expect(
        await GitOps(
          const _CannedGit('fatal: not a repo', exitCode: 128),
  ...
  group('the completion fence excludes the grid runtime dir', () {
    ...
    test(
      'nested grid artifacts (critique / spec / telemetry / pinned diff) are '
      'not work',
      () async {
        const porcelain =
    ...
    test(
      'a path merely PREFIXED by the excluded name is work (`.gridlock/x`)',
      () async {
        expect(
  ...
  group('a WARNING on stderr is never parsed as a change', () {
    ...
    test('exit 0 + a stderr warning over a CLEAN tree is probeError, NOT a '
      ...
      expect(
        await GitOps(
          const _CannedGit(warning, stderr: warning),
        ).hasUncommittedWork('/w', excluding: const {'.grid'}),
        GateOutcome.probeError,
      );
    });
    ...
    test(
      'exit 0 + a stderr warning ALONGSIDE real porcelain is still probeError '
    ...
    test('the reap gate ALSO refuses on a degraded scan (probeError blocks, '
      ...
      expect(
        ...
        isTrue,
        reason:
            'ADR-0006 D3: a reap must still refuse — the decision is '
