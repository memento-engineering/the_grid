...
void main() {
  ...
  test(
    'mints a type=gate bead + stamps {rig, blocks, node, reason} from birth',
    () async {
  ...
  test(
    'mint-dedup (tg-i08): an existing OPEN gate for the SAME (session, node) '
    'is REFRESHED, not re-minted',
    () async {
  ...
  test(
    'mint-dedup does NOT reuse a CLOSED gate (open-only) — mints fresh',
    () async {
      runner.exportBeads = [
        Bead(
          id: 'tgdog-gclosed',
  ...
  test('fail-closed: a gate for a CLOSED session is refused before create or '
      'refresh', () async {
    runner.exportBeads = [session('tgdog-s', closed: true)];
    ...
    await expectLater(
      writer().createGate(
        ...
        sessionId: 'tgdog-s',
        nodePath: 'tg-1/review/route',
        reason: 'route re-gated after session close',
      ),
      throwsA(
        isA<SessionClosedRefused>()
            .having((e) => e.operation, 'operation', 'create')
            .having((e) => e.sessionId, 'sessionId', 'tgdog-s'),
      ),
    );
