...
void main() {
  group('InheritedCircuit', () {
    ...
    test('== ignores beadIdByNodePath — a same-state re-provide with a freshly '
    ...
    test(
      '!= when a node in the cursor moves state (a real state change notifies)',
      () {
        ...
        final after = InheritedCircuit(
          ...
          cursor: {
            ...cursorA,
            'build': const NodeCursor(state: StepState.failed),
          },
        );
    ...
    test('a step resolves its own bead id by nodePath through '
    ...
    test(
      ...
      () {
        ...
        expect(
          ctx.dependOnInheritedSeedOfExactType<InheritedCircuit>(),
          isNull,
        );
      },
    );
