...
| `attempt.terminal` (settled) | `UnknownTerminalSettlementObligation` is the shipped settlement owner; `settleSessionForTerminalWork` only supplies ledger state and makes no recorder call | outcome=settled, work_terminal_reason; `inferred` is reserved for this probe-backed settlement. `reconstructed` is reserved for external-close healing, per `the_grid#terminal-provenance-word-is-reconstructed`. |
