...
register:
  spec: 1
  slug: adr-0001-technical-foundations
  surfaces:
    - "packages/**"
  obsoletes: []
  updates: []
...
**The wire key is `extensions`, not `plugins`** *(amended per ADR-0000 A33, ratified 2026-06-15; shipped in M3 Track 6)*: leonard's published reader (≥0.1.0) reads the handshake/observation map under **`extensions`** with no fallback, so the_grid's host emits `extensions: [{namespace: "grid", tools: […]}]` and nests grid state under **`extensions.grid`**. The `ext.exploration.*` prefix, the method names, and the protocol version are unchanged — a pure key-name convergence onto leonard's contract, no protocol-version bump. Read `plugins`/`plugins.grid` above as `extensions`/`extensions.grid`.
...
**The namespace is `ext.leonard.*`, owned by leonard** *(amended per ADR-0000 A56, ratified by Nico 2026-08-01)*: the VM-service namespace belongs to leonard — the contract authority publishing the protocol as `leonard_contract` — and the_grid is a conforming implementer, not the owner. Methods are `ext.leonard.core.handshake`, `ext.leonard.core.get_stable_observation`, and `ext.leonard.grid.<tool>`; the protocol version is `2`. Read the `ext.exploration.*` examples above as `ext.leonard.*`. The grid's constants import from `leonard_contract` (landed tg-4a78 → tg-99rr; the interim local re-declaration is deleted). The lenny-side rename landed as bead `lenny-uoiw` (`lenny-wisp-41rdl` above is a gc-era tombstone). The `extensions` wire key and method suffixes are unchanged.
