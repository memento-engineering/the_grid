...
register:
  spec: 1
  slug: adr-0007-tree-engine-and-genesis-supersession
  surfaces:
    - "packages/**"
  obsoletes: []
  updates: []
...
- **Amended 2026-08-08 (B6 — ratified Nico; quote-and-supersede):** D-1's “`bd update --metadata` is a lockless client-side read-modify-write” rationale is superseded by upstream #4732's transactional metadata merge; `StationBeadWriter._tail` remains defense-in-depth and the per-target-id ordering guarantee, while ownership-sensitive updates use `--if-assignee`/`--if-status` CAS and surface a guard mismatch as typed `OwnershipGuardRefused`, never a blind retry.
