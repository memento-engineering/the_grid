...
register:
  spec: 1
  slug: adr-0008-authoring-sdk-and-reentrant-engine
  surfaces:
    - "packages/**"
  obsoletes: []
  updates: []
...
**Amended 2026-07-02 (ratified Nico — the agent-scope design pass, `docs/SCRATCH-agent-scope.md` (retired to git history — tg-8gv.8)):** Decisions 2/3/4(D-6)/9 carry quote-and-supersede amendments and **Decision 10 (the agent scope) is added**; stamps applied same-day, never silent.
**Vocabulary superseded 2026-07-05 (stamp; rename ratified as RS-7, `docs/SCRATCH-resident-station.md` §6/§9; reaffirmed Nico 2026-07-05, GLOSSARY review R8):** this ADR's **`Formula` vocabulary reads as `Circuit`** throughout — `Formula`→`Circuit`, `FormulaStep`→`CircuitStep`, `FormulaScope`→`CircuitScope`, `FormulaResolver`→`CircuitResolver`; the engine verb is **energize**. The rename is fully applied in code (`grid_engine` has zero `Formula` symbols). "Formula" survives only in gas-city transition/compat prose explaining circuits' lineage. The decisions below are unchanged in substance; only the noun moved.
**Decision 1 naming revised (R9, Nico 2026-07-05):** the public SDK package name is **`grid_sdk`** (the framework as concrete code); **"the grid"** remains the framework as a *concept* (and `the_grid` the repo). D1's `the_grid`-as-package-name (with `grid_sdk` as fallback) is superseded — the fallback won.
...
**Source of record:** `docs/SCRATCH-vnext-prd.md` (retired to git history — tg-8gv.8) §§0.5–4, §9 (the design surface; this ADR promotes its converged decisions and states the load-bearing ones).
...
This ADR decides the **single-station authoring + engine-extension cluster** (the build target). It deliberately does **not** decide **federation** (the multi-station story — `docs/SCRATCH-vnext-prd.md` (retired to git history — tg-8gv.8) §§5–7, §9 impls) or **observability** (§8); those graduate to their own ADRs whose **numbers Nico assigns** (they do not cleanly fit ADR-0007 §5's tentatively-reserved P2/P3). Where state restoration (Decision 6) ultimately *lives* is a genesis-side decision, deferred.
...
**Amended 2026-07-20 (public-readiness `tg-8gv.8`) — Status: records the ratified decision `space-6ds`.** Nico ratified, 2026-07-10, how a station composes its own substation roster (graduated from the now-deleted memento-composition design scratch; anchor bead `space-6ds` filed in the `space` store, deferred). Two forks, both resolved:

- **Roster form — literal, hardcoded `Substation()` instantiation.** Not a manifest, not a `for`-loop, not filesystem discovery. A composing station authors each substation as a direct `Substation(...)` call in its `SpaceDelegate.build()`-equivalent, with sibling-relative roots (e.g. `../genesis`, `../the_grid`) baking in the opinion that the composed repos sit side-by-side in the umbrella checkout. For space_station specifically, this means `genesis` + `the_grid` + `power_station` + `space_station` + `lenny` as five literal `Substation()` calls.
- **Config layers append/merge onto the coded base, never replace it.** The hardcoded roster is the base/default. Parameterized configs — `--substation` flags today, TOML eventually — append new substations onto that base or merge overrides into a coded one by name. A no-flag `station up` arms the full coded roster.

This is an authoring-model decision under this Decision's Station/Substation vocabulary, not a change to the vocabulary itself.
...
- **`Trust`** (local / reputation / ledger) — the admission-gate decision ("do I believe this result/peer enough to admit it?"); impls plug in (`docs/SCRATCH-vnext-prd.md` (retired to git history — tg-8gv.8) §9). Distinct from **`consent`** (`genesis_consent`, *authorization* — "may this actor act?"). Trust's interface home (the_grid vs a genesis-shared abstraction) is open.
...
**Reality stamp (2026-07-19, public-readiness pass):** "adopt is the floor" is now shipped code — `startOrAdopt` (`grid_engine/lib/src/sdk/lease.dart`) + the `AdoptProof` seam (`grid_engine/lib/src/restart/restart_reconciler.dart`), with ADR-0009 D4 making the lifecycle (`startOrAdopt`/`update`/`dispose`/`detach`) type properties of the Allocation. And on the molecule path (DESIGN-tg-pm6 R3), the pgid/pid/token restart fence persists in the vendor-owned `grid.lease.*` namespace on the step bead — written only by `StationProcessLeaseVendor` (`tg-h4u` #62; mounted at the production work seat by `tg-2mb` #73) — the `grid.cursor.*` fence remaining flat-path-only.
...
**Amended 2026-07-02 (ratified Nico; `docs/SCRATCH-agent-scope.md` (retired to git history — tg-8gv.8) D-D) — the sibling-read seam goes AMBIENT:** `SiblingView` is mounted by the session/fan-out container and read via the non-binding lookup (Decision 3's 2026-07-02 amendment) instead of being threaded through the now-deleted `CapabilityContext`. Still read-only, still no pipeline subscription — the seam's spirit is unchanged; only the plumbing moved.

---

## Decision 10 — the agent scope: harnesses over the agentic step

**Decided 2026-07-02 (ratified Nico; design surface `docs/SCRATCH-agent-scope.md` — its D-A…D-H are the detail of record).** **[2026-07-20 — tg-8gv.8: that design surface is retired to git history; this ADR and the git-history copy jointly hold the D-A…D-H detail.]**
...
- **The genesis_tree consumption doctrine** (`docs/SCRATCH-agent-scope.md` (retired to git history — tg-8gv.8) D-H) is ratified house style: always watch dependencies (never `??=`-cache — genesis's `dependencyChanged` must re-run the read); never sync-read reactive state without subscribing (no public state mirrors); no service-layer access in seeds/branches except DI/composition — branches touch value types and reactive domain objects; delegate-classes (pure description) are acceptable in build; I/O services flow untouched to the effect boundary. Propagation to predictable-flutter/the genesis docs is Nico's.
...
- **Federation** — `Station`/`Substation`/virtual, the per-station no-center `Grid`, the pull predicate, dolt + the inter-station bus, claim mechanics, presence/reaping, and the `Trust` *impls* (`docs/SCRATCH-vnext-prd.md` (retired to git history — tg-8gv.8) §§5–7, §9).
- **Observability** — observable-source as a first-class engine concept, OTel ⊥ perception (both sinks on the reconcile-event stream), the AOT exploration transport (`docs/SCRATCH-vnext-prd.md` (retired to git history — tg-8gv.8) §8).
