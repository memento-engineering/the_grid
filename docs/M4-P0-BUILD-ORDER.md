...
- **Type exclusion BEFORE ownership:** exclude `IssueType.convergence` (and other M2/M3-owned infra types) at the mount boundary — a `type=convergence` root that the_grid owns must mount **zero** WorkBeads (it's the M2 `ReconcilerRuntime`'s exclusively, on the `convergence.state` axis — mounting an `AgentEffect` on it is a true two-writer collision). *(This also patches a latent gap in shipped M3 `dispatch_interactor.dart` — carry-forward note.)*
